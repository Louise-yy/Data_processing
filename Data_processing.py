...
# # 2.筛选属于特定文件夹中的行，并把选取的帧选出来！P02_108中的stop_frame已改
# df = pd.read_csv("file/front20.csv")
# # selected_rows = df[df['narration_id'].str.startswith(('P01_102', 'P02_108', 'P02_109', 'P02_112', 'P02_129',
# #  'P04_113', 'P04_120', 'P04_121', 'P22_117'))]
# selected_rows = df[df['narration_id'].str.startswith('P04_120')]  # ################################
# print(selected_rows.shape[0])
# # 筛选需要的列
# selected_rows.loc[:, 'stop_frame'] = (selected_rows['start_frame'] + selected_rows['stop_frame']) // 2
# selected_columns = selected_rows.iloc[:, [0, 2, 3, 6, 7]]
# # 输出
# selected_columns.to_csv('file/P04_120.csv', index=False)  # ##############################


# # 3.选取的帧的名字从11074改成./frame_0000011074.jpg
# file_path = "file/P04_120.csv"  # ####################################
# # 读取CSV文件
...
# # 保存修改后的数据到CSV文件
# df.to_csv('file/P04_120(2).csv', index=False)  # #################################


# # 4.从tar压缩包中提取与CSV文件中stop_frame列的数据对应的照片
# tar_file_path = '/media/louise/UBUNTU 20_0/yy/P04_120.tar'  # ############################
# csv_file_path = 'file/P04_120(2).csv'  # ########################
# df = pd.read_csv(csv_file_path)
# output_folder = 'P04_120'  # ##########################
...
# csv_file_path = 'file/P04_120.csv'  # ###########################
# df = pd.read_csv(csv_file_path)
...
# df = pd.read_csv(file_path)
# # 提取数字部分
# df['stop_frame'] = 'P04_120_' + df['stop_frame'].astype(str) + '.jpg'  # ##############################
# # print(df['stop_frame'])
# # 保存修改后的数据到CSV文件
# df.to_csv('file/P04_120(2).csv', index=False)  # ###############################
...
# # 将所有的（2）文件合成为一个
# # 要合并的CSV文件路径
# csv_files = glob.glob('/media/louise/UBUNTU 20_0/yy/Data_processing/file/data_file/*.csv')
#
# # 读取并合并CSV文件
# merged_df = pd.concat([pd.read_csv(file) for file in csv_files], ignore_index=True)
# selected_columns = merged_df.iloc[:, [1, 3, 4]]
#
# # 输出合并后的数据框
...
# 按 stop_frame 列进行分组，并将all_nouns和all_noun_classes 列的值合并为逗号分隔的字符串
...
df_grouped.to_csv('file/label_special_form.csv', index=False)
